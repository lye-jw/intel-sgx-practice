 ...
 */
static bool seal_and_save_data(const char *plaintext)
{
    ...
    ret = get_sealed_data_size(eid_seal, &sealed_data_size, plaintext);
    if (ret != SGX_SUCCESS)
    {
        ret_error_support(ret);
    ...
    sgx_status_t retval;
    ret = ecall_seal_data(eid_seal, &retval, plaintext, temp_sealed_buf, sealed_data_size);
    if (ret != SGX_SUCCESS)
    {
        ret_error_support(ret);
