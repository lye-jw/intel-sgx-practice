...
static sgx_status_t SGX_CDECL sgx_get_sealed_data_size(void* pms)
{
	...
	size_t _len_plaintext = ms->ms_plaintext_len ;
	...
	if (_tmp_plaintext != NULL && _len_plaintext != 0) {
		...
		_in_plaintext[_len_plaintext - 1] = '\0';
		if (_len_plaintext != strlen(_in_plaintext) + 1)
		{
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	ms->ms_retval = get_sealed_data_size((const char*)_in_plaintext);
...
static sgx_status_t SGX_CDECL sgx_ecall_seal_data(void* pms)
{
	...
	size_t _len_plaintext = _tmp_data_size;
	char* _in_plaintext = NULL;
	...
	ms->ms_retval = ecall_seal_data((const char*)_in_plaintext, _in_sealed_blob, _tmp_data_size);
	if (_in_sealed_blob) {
