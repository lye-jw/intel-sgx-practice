...
sgx_status_t enclave_unseal_key(uint8_t *decrypted_data, const uint8_t *sealed_blob, size_t data_size)
{
    ...
    mac_text_len = sgx_get_add_mac_txt_len((const sgx_sealed_data_t *) sealed_blob);
    decrypt_data_len = sgx_get_encrypt_txt_len((const sgx_sealed_data_t *) sealed_blob);
    ...
    uint8_t de_mac_text[mac_text_len + 1];

    ret = sgx_unseal_data((const sgx_sealed_data_t *) sealed_blob, de_mac_text, &mac_text_len,
