...
static sgx_status_t SGX_CDECL sgx_get_sealed_data_size(void* pms)
{
	...
	const char* _tmp_ori_data = ms->ms_ori_data;
	size_t _len_ori_data = ms->ms_ori_data_len ;
	char* _in_ori_data = NULL;

	CHECK_UNIQUE_POINTER(_tmp_ori_data, _len_ori_data);
	...
	if (_tmp_ori_data != NULL && _len_ori_data != 0) {
		_in_ori_data = (char*)malloc(_len_ori_data);
		if (_in_ori_data == NULL) {
			status = SGX_ERROR_OUT_OF_MEMORY;
			goto err;
		}

		if (memcpy_s(_in_ori_data, _len_ori_data, _tmp_ori_data, _len_ori_data)) {
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}

		_in_ori_data[_len_ori_data - 1] = '\0';
		if (_len_ori_data != strlen(_in_ori_data) + 1)
		{
			status = SGX_ERROR_UNEXPECTED;
			goto err;
		}
	...
	ms->ms_retval = get_sealed_data_size((const char*)_in_ori_data);

err:
	if (_in_ori_data) free(_in_ori_data);
	return status;
}

static sgx_status_t SGX_CDECL sgx_ecall_seal_aes_key(void* pms)
{
	CHECK_REF_POINTER(pms, sizeof(ms_ecall_seal_aes_key_t));
	//
	// fence after pointer checks
	//
	sgx_lfence();
	ms_ecall_seal_aes_key_t* ms = SGX_CAST(ms_ecall_seal_aes_key_t*, pms);
	sgx_status_t status = SGX_SUCCESS;
	...
	ms->ms_retval = ecall_seal_aes_key(_in_sealed_blob, _tmp_data_size);
	if (_in_sealed_blob) {
...
SGX_EXTERNC const struct {
	...
	2,
	{
		...
		{(void*)(uintptr_t)sgx_ecall_seal_aes_key, 0, 0},
	}
};
...
sgx_status_t SGX_CDECL ocall_print_byte(char c)
{
	...
	ms_ocall_print_byte_t* ms = NULL;
	size_t ocalloc_size = sizeof(ms_ocall_print_byte_t);
	void *__tmp = NULL;
	...
	ms = (ms_ocall_print_byte_t*)__tmp;
	__tmp = (void *)((size_t)__tmp + sizeof(ms_ocall_print_byte_t));
	ocalloc_size -= sizeof(ms_ocall_print_byte_t);
